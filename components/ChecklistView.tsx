...
import { getTotalItems } from "@/data/checklists";
...
export default function ChecklistView({ checklist, categoryName, categoryIcon }: Props) {
  const storageKey = `checklist_${checklist.slug}`;
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-slate-500 mt-2">{checklist.description}</p>
      </div>
      ...
      <div className="space-y-8">
        {checklist.sections.map((section, sectionIndex) => {
          const sectionItems = hideCompleted
            ? section.items.filter((item) => !checked.has(item.id))
            : section.items;

          const sectionDone = section.items.filter((item) => checked.has(item.id)).length;
          const sectionTotal = section.items.length;

          if (hideCompleted && sectionItems.length === 0) return null;
      ...
      {/* All done message */}
      {hideCompleted && checklist.sections.every((s) => s.items.every((item) => checked.has(item.id))) && (
